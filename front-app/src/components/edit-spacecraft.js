...
const EditSpacecraft = (props) => {
  const { setShowModal, spacecraft } = props;
  const [name, setName] = useState(spacecraft.name);
  const [maximumSpeed, setMaximumSpeed] = useState(spacecraft.maximumSpeed);
  const [mass, setMass] = useState(spacecraft.mass);

  const editSpacecraft = async () => {
    const bodyValues = {
      name: name,
